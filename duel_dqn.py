...
# Configure logging to write to both a file and output to the terminal.
logging.basicConfig(
    level=logging.INFO,
...
class model(nn.Module):
    def __init__(self, n_frame, n_action, device):
        ...
        # First convolutional layer (input: 4 frames, output: 32 feature maps)
        self.layer1 = nn.Conv2d(n_frame, 32, 8, 4)
        # Second convolutional layer (input: 32 feature maps, output: 64 feature maps)
        self.layer2 = nn.Conv2d(32, 64, 3, 1)
        # Fully connected layer (input: 20736, output: 512)
        self.fc = nn.Linear(20736, 512)
        # Action advantage output layer (calculates A(s, a))
        self.q = nn.Linear(512, n_action)
        # State value output layer (calculates V(s))
    ...
    def forward(self, x):
        ...
        # Compute the action advantages (A(s, a)) and the state value (V(s))
        adv = self.q(x)
        v = self.v(x)

        # Compute Q-values using Dueling DQN formula
...
def main(env, q, q_target, optimizer, device):
    ...
    for k in range(1000000):
        ...
        if k % print_interval == 0:
            ...
            start_time = time.perf_counter()
            log_msg = (
                f"{device} | Epoch : {k} | score : {total_score / print_interval:.6f} | "
                f"loss : {loss / print_interval:.2f} | stage : {stage} | time spent: {time_spent:.6f}"
            )
